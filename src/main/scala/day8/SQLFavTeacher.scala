...
object SQLFavTeacher {
    def main(args: Array[String]): Unit = {
        ...
        //窗口函数 over(order by counts desc) = 全局排序
        //val temp2 = spark.sql("SELECT subject, teacher, counts, row_number() over(partition by subject order by counts desc)  g_rk FROM v_temp_sub_teacher_counts")
